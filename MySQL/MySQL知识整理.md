...
解读：a、这两种类型的都能表达`"yyyy-MM-dd HH:mm:ss"`格式的时间，TIMESTAMP只需要占用4个字节的长度，可以存储的范围为`(1970-2038)`年，在各个时区，所展示的时间是不一样的；b、而`DATETIME`类型占用8个字节，对时区不敏感，可以存储的范围为`(1001-9999)`年。
...
解读：a）读取不需要的列会增加`CPU、IO、NET`消耗 b）不能有效的利用覆盖索引
