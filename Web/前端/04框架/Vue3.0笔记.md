...
```vue
<template>
  <h1>{{ $translate('greetings.hello1.Bonjour') }}</h1>
  <h1>{{ translate('greetings.hello') }}</h1>
</template>
...
import { inject } from 'vue'
// 使用插件
const { translate, il8n } = inject('il8n') as {
  translate: (key: string) => string;
  il8n: Record<string, any> | undefined;
}
console.log(il8n?.greetings.hello1.Bonjour)
</script>
```
...
[^PascalCase]:**`PascalCase`**‌是一种编程中的命名约定，主要用于类名、接口名、枚举名和异常名的命名。所有单词的首字母大写，并且单词之间没有连接符。
[^camelCase]:`camelCase` 是一种命名约定，用于标识变量、函数名等。第一个单词的首字母小写，其余单词的首字母大写。
[^kebab-case]:`kebab-case` 是一种命名约定，它由一个或多个单词组成，中间用短横线（-）连接
