...
在UTC(又名GMT)中，从Epoch开始的秒数。它可以是一个整数
or a floating point number (to represent fractions of seconds).
或者一个浮点数(表示秒的分数)。
The Epoch is system-defined; on Unix, it is generally January 1st, 1970.
Epoch是系统定义的;在Unix上，通常是1970年1月1日。
The actual value can be retrieved by calling gmtime(0).
实际值可以通过调用gmtime(0)来获取。

The other representation is a tuple of 9 integers giving local time.
另一种表示是一个由9个整数组成的元组，给出了当地时间。
The tuple items are:
元组项是:
...
def asctime(p_tuple=None): # real signature unknown; restored from __doc__  真实签名未知;从文档恢复
    """
    ...
    将时间元组转换为字符串，例如:'星期六六月06 16:26:11 1998'。
    当时间元组不存在时，localtime()返回的当前时间使用。
    """
    return ""

def ctime(seconds=None): # known case of time.ctime  已知情况下的时间
    """
    ...
    将从Epoch开始以秒为单位的时间转换为本地时间的字符串。
    这相当于asctime(localtime(seconds))。当时间元组是不存在，使用localtime()返回的当前时间。
    """
    return ""
...
def gmtime(seconds=None): # real signature unknown; restored from __doc__
    ...
    将Epoch以来的秒数转换为表示UTC的时间元组(也称为UTC)。如果没有传入'seconds'，则转换当前时间。
	如果平台支持tm_gmtoff和tm_zone，则它们可以作为属性。
    """
    pass
