...
- 通过`interpolate()`方法填补缺失值。对于连续性较强的数据，插值能够根据已有数据点推算出缺失值。常见的插值方法包括线性插值、时间插值、多项式插值等。
...
df = pd.DataFrame(
    [["2024-05-21 01:01:01", "2024-05-21 04:01:01"],], columns=["a", "b"])
df = df.astype("datetime64[ms]")
df["c"] = (df["a"]-df["b"]).dt.total_seconds() /3600
...
| xlwt      | √    | √    | √    | √    |      | √    | ×     |       | ×    | √    | √    |                        |
| xlutils   | √    | √    | √    | √    |      | √    | ×     |       | ×    | ×    | √    |                        |
| xlwings   | √    | √    | √    | √    |      | √    | √     |   √   | √    | √    | √    | 电脑必须安装office软件 |
| openpyxl  | √    | ×    | √    | √    |      | ×    | √     |       | √    | √    | √    |                        |
| xlswriter | √    | √    | √    | √    |      | ×    | √     |       |      | √    | ×    |                        |
| win32com  | √    | √    | √    | √    |      | √    | √     |   √   | √    | √    | √    | 电脑必须安装office软件 |
| DataNitro | √    | ×    | √    | √    |      | √    | √     |       | —    | —    | —    |                        |
| pandas    | √    | √    | √    | √    |      | √    | √     |       | √    | √    | ×    |                        |
...
![image-20240318171333486](imge/Pandas基础.assets/image-20240318171333486.png)
...
![冒泡排序](imge/Pandas基础.assets/冒泡排序.gif)
...
```python
def shell_sort(lists: list):
    count = len(lists)
    step = 2
    grop = int(count / step)
    while grop > 0:
        for i in range(grop, count):
            temp = lists[i]
            j = i
            while j >= grop and lists[j - grop] > temp:
                lists[j] = lists[j - grop]
                j -= grop
            lists[j] = temp
        grop = int(grop / step)
    return lists
...
![选择排序](imge/Pandas基础.assets/选择排序.gif)
...
![基数排序](imge/Pandas基础.assets/基数排序.gif)
